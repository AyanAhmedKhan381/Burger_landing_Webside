import Imag1 from "../../../../public/images/f1.jpg";
import Img2 from "../../../../public/images/f2.jpg";
import Img3 from "../../../../public/images/f3.jpg";
import Image from "next/image";

const Feature = () => {
  return (
    <div className="pt-[7rem] pb-[4rem]">
      {/* Heading */}
      <h1 className="heading text-center text-6xl">
        Burgers made with <br /> love and <span className="text-red-600">Care</span>
      </h1>

      {/* Cards */}
      <div className="w-[90%] md:w-[80%] mt-[3rem] md:mt-[5rem] mb-[3rem] mx-auto grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-[3rem]">
        
        {/* 1st card */}
        <div data-aos="fade-left" data-aos-anchor-placement="top-center">
          <div className="p-6 hover:bg-white rounded-lg transition-all duration-200">
            {/* Image */}
            <Image src={Imag1} alt="Image 1" className="rounded-3xl" />
            
            <h1 className="mt-[1.5rem] text-center text-[24px] text-black font-semibold">
              Our Burger
            </h1>
            <p className="mt-[0.2rem] text-black text-opacity-60 text-center">
              Savor our juicy, flame-grilled burger, topped with fresh lettuce, ripe tomatoes, crispy pickles, and melted cheese, all nestled in a soft, toasted bun. A mouthwatering experience you wont forget!
            </p>
          </div>
        </div>
        

        {/* 2nd card */}
        <div data-aos="zoom-in" data-aos-delay="200" data-aos-anchor-placement="top-center">
          <div className="p-6 hover:bg-white rounded-lg lg:translate-y-[3rem]  transition-all duration-200">
            <Image src={Img2} alt="Image 2" className="rounded-3xl" />
            {/* Text */}
            <h1 className="mt-[1.5rem] text-center text-[24px] text-black font-semibold">
              Your Opinion is Important
            </h1>
            <p className="mt-[0.2rem] text-black text-opacity-60 text-center">
              Savor our juicy, flame-grilled burger, topped with fresh lettuce, ripe tomatoes, crispy pickles, and melted cheese, all nestled in a soft, toasted bun. A mouthwatering experience you wont forget!
            </p>
          </div>
        </div>
        

        {/* 3rd card */}
        <div data-aos="fade-right" data-aos-delay="300" data-aos-anchor-placement="top-center">
          <div className="p-6 hover:bg-white rounded-lg transition-all duration-200">
            <Image src={Img3} alt="Image 3" className="rounded-3xl" />
            {/* Text */}
            <h1 className="mt-[1.5rem] text-center text-[24px] text-black font-semibold">
              Chicken Burgers
            </h1>
            <p className="mt-[0.2rem] text-black text-opacity-60 text-center">
              Savor our juicy, flame-grilled burger, topped with fresh lettuce, ripe tomatoes, crispy pickles, and melted cheese, all nestled in a soft, toasted bun. A mouthwatering experience you wont forget!
            </p>
          </div>
        </div>

      </div>
      
    </div>
  );
};

export default Feature;
